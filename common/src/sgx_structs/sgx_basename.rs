...
impl SgxStruct for SgxBasename {
    ...
    /// C/C++ format.
    ///
    /// NOTE: If len(self.name) is less than self.STRUCT_SIZE, the resulting bytes will be padded
    /// with binary zero (\x00).  If len(self.name) is greater than self.STRUCT_SIZE, the
