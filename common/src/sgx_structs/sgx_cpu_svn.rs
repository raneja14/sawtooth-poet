...
impl SgxStruct for SgxCpuSvn {
    ...
    /// C/C++ format.
    ///
    /// NOTE: If len(self.svn) is less than self.STRUCT_SIZE, the resulting bytes will be padded
    /// with binary zero (\x00). If len(self.svn) is greater than self.STRUCT_SIZE, the resulting
