...
impl SgxStruct for SgxReportData {
    ...
    /// C/C++ format.
    ///
    /// NOTE: If len(self.d) is less than self.STRUCT_SIZE, the resulting bytes will be padded
    /// with binary zero (\x00). If len(self.d) is greater than self.STRUCT_SIZE, the resulting
