...
pub fn init_enclave(
    ...
    unsafe {
        ...
        let enclave_init_status =
            r_initialize_enclave(eid_ptr, enclave_cstring.as_ptr(), spid_cstring.as_ptr());
...
pub fn set_sig_revocation_list(
    ...
    unsafe {
        ...
        let sig_revocation_status =
            r_set_signature_revocation_list(eid_ptr, sig_rev_list_cstring.as_ptr());
...
pub fn create_signup_info(
    ...
    unsafe {
        ...
        let signup_status =
            r_create_signup_info(eid_ptr, opk_hash_cstring.as_ptr(), signup_info_ptr);
...
pub fn initialize_wait_cert(
    ...
    unsafe {
        ...
        let wait_cert_init_status = r_initialize_wait_certificate(
            eid_ptr,
            duration_ptr,
...
pub fn finalize_wait_cert(
    ...
    unsafe {
        ...
        let wait_cert_final_status = r_finalize_wait_certificate(
            eid_ptr,
            wait_cert_ptr,
            prev_wait_cert_cstring.as_ptr(),
...
pub fn verify_wait_certificate(
    ...
    unsafe {
        ...
        let status = r_verify_wait_certificate(
            eid_ptr,
            ppk_cstring.as_ptr(),
...
pub unsafe fn create_string_from_char_ptr(cchar_ptr: *mut ::std::os::raw::c_char) -> String {
    let c_str: &CStr = CStr::from_ptr(cchar_ptr);
    let str_slice: &str = c_str
        .to_str()
        .expect("Unable to read string from cchar ptr");
    let string_buf: String = str_slice.to_owned();

    string_buf
}

pub fn release_signup_info(
    eid: &mut r_sgx_enclave_id_t,
    signup_info: &mut r_sgx_signup_info_t,
) -> Result<String, String> {
    unsafe {
        let eid_ptr = eid as *mut r_sgx_enclave_id_t;
        let signup_info_ptr = signup_info as *mut r_sgx_signup_info_t;
        let singup_release_status = r_release_signup_info(eid_ptr, signup_info_ptr);
...
pub fn release_wait_certificate(
    eid: &mut r_sgx_enclave_id_t,
    ...
    unsafe {
        ...
        let wait_cert_release_status = r_release_wait_certificate(eid_ptr, wait_cert_ptr);
